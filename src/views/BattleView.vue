<template>
        ...
        <div class="row">
          <div class="col-6 d-flex align-items-center justify-content-between">
            <div class="d-flex flex-column justify-content-center gap-2 actions mb-4">
              <p class="outlined-text">Niveau {{ player.level }} <br> Expérience : {{ player.experience }}/{{ player.experienceToNextLevel }}</p>
              <button
  ...
  export default {
    ...
    created() {
      ...
      axios.get(`http://localhost:3000/api/character?name=${characterName}`)
        .then(response => {
      ...
      axios.get('http://localhost:3000/api/enemy?name=goblin')
        .then(response => {
